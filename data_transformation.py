...
def data_cleaning_process():
    ...
    for col_name in filtered_orders.columns:
        if type(filtered_orders[col_name][0]) == dict:
            filtered_orders[col_name] = filtered_orders[col_name].apply(
                lambda x: x["$oid"] if isinstance(x, dict) and "$oid" in x else x
            )
    ...
    filtered_orders["unidades_vendidas"] = filtered_orders["unidades_vendidas"].astype(
        int
    )
    ...
    return print(
        f"se insertaron {len(df_json)} documentos a la DB warehouse a las {col_hour} en UTC-5"
    )
...
def cronjob_log():
    ...
    return print(
        f"ejecución del chequeo de salud para el cronjob fue realizada a las : {col_hour} en UTC-5"
    )
